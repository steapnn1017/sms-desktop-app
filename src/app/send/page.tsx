...
const schema = z.object({
  phone: z.string().min(1, 'Zadejte telefonní číslo').refine(
    (v) => validatePhone(v),
    'Neplatný formát čísla (např. +420 777 123 456)'
  ),
...
export default function SendSmsPage() {
  ...
  // Auto-generate message when template or variables change
  ...
  // Set preset template
  useEffect(() => {
    if (presetTemplateId && templates.length > 0) {
      setValue('templateId', presetTemplateId)
    }
  ...
  const onSubmit = async (data: FormData) => {
    if (!isElectron) {
      toast.error('Nejste v Electron prostředí')
      return
    }
    ...
    try {
      ...
      if (result.success) {
        toast.success('SMS úspěšně odeslána!', {
          description: `Na číslo ${formatPhone(normalizePhone(data.phone))}`,
        })
        setSent(true)
        setTimeout(() => {
          setSent(false)
        ...
        window.electronAPI.notification.show(
          'SMS odeslána',
          `Zpráva na ${formatPhone(normalizePhone(data.phone))} byla odeslána.`
        )
      } else {
        toast.error('Chyba při odesílání SMS', {
    ...
    <div className="max-w-2xl space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-[#525252] mt-0.5">
          Vyberte šablonu a vyplňte údaje zakázky
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
        ...
        <div className="app-card p-5 space-y-4">
            ...
            {templates.length === 0 ? (
              ...
              templates.map((tmpl) => (
                ...
                >
                  ...
                  {tmpl.description && (
                    <p className="text-[10px] mt-0.5 opacity-60">{tmpl.description}</p>
                  )}
                </button>
              ))
            )}
          ...
          {errors.templateId && (
            <p className="text-xs text-red-400">{errors.templateId.message}</p>
          )}
        </div>
        ...
        <div className="app-card p-5 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2 text-xs text-[#525252]">
              <span className={cn(charCount > 160 && 'text-yellow-400')}>
                {charCount} znaků
              </span>
              <span>•</span>
          ...
          <textarea
            {...register('message')}
            rows={4}
            placeholder="Text zprávy se vygeneruje automaticky po výběru šablony..."
            className={cn(
              'app-input resize-none font-mono text-sm leading-relaxed',
          ...
          <p className="text-xs text-[#525252]">
            Zpráva se automaticky vygeneruje z vybrané šablony. Můžete ji ručně upravit.
          </p>
        </div>
